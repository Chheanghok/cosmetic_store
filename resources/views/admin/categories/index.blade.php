<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap justify-between items-center">
            ...
            <a href="{{ route('categories.create') }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded mt-2 sm:mt-0">
                Add New Category
            </a>
        </div>
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form method="GET" action="{{ route('categories.index') }}" class="mb-4">
                        <div class="flex flex-col sm:flex-row gap-4">
                            <div class="flex-grow">
                                <input type="text" name="search" placeholder="Search by category name..." value="{{ request('search') }}" class="w-full rounded-md shadow-sm border-gray-300">
                            </div>
                            <div>
                                <button type="submit" class="w-full bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded">Filter</button>
                            </div>
                            <div>
                                <a href="{{ route('categories.index') }}" class="w-full block text-center bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">Clear</a>
                            </div>
                        </div>
                    </form>
